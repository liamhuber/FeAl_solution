...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/structures}
...
Although smaller than the experimental dimensions, the simulation cells were of the same order of magnitude: $4\times4\times4$ nanometer periodic cubes, i.e. 5488 atoms in a periodic domain.
The convergence of the system to the desired chemical ordering can be seen by the good equilibration of the potential energy as a function of time in Fig.~\ref{fig:energy_conv} for all systems except for the Lee potential \todo{potl2} at 523~K.
...
\begin{figure}[h]
    ...
    \caption{Total system potential energy convergence as a function of time (and thus MC swaps) for \todo{Mendelev=potl0, Lee=potl2}.}
...
Such a strict threshold also minimizes the overlap between clusters of the same type (i.e. \DOTHREE or \BTWO) which beneficial since we otherwise ignore this effect in our accounting.
The interested reader can examine the \code{github} repository~\cite{feal} for more detail.

The resulting histograms for identified clusters -- ignoring single atom ``clusters'' and binning by ten atoms at a time -- can be seen in Figs.~\ref{fig:mendelev_hist} and \ref{fig:lee_hist} for the Mendelev and Lee potentials, respectively.
The Mendelev potential shows a very mild decrease in \DOTHREE clusters and increase in \BTWO clusters, but it is clear from examining the axes that this is trivial -- at neither temperature and for neither reference phase does the Mendelev potential exhibit SRO.
In contrast, the Lee potential shows a distinct tendency to form clusters with \DOTHREE ordering, especially at higher temperatures (and no tendency to have \BTWO ordering).
This high temperature behaviour is fully consistent with the simplified phase diagram in Fig.~\ref{fig:expanded_0K_phases}, where a high-Al-concentration solid solution and \DOTHREE have some coexistence, and qualitatively with experimental results.
%
\begin{figure}[h]
    \centering
...
Can we also test whether Fig.~\ref{fig:expanded_0K_phases} makes accurate predictions for the Mendelev potential?
...
Partial columns of Al are also visible in the full structure, also shown in Fig.~\ref{fig:mendelev_structure}(b).
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/equilibrated_mendelev_523}
